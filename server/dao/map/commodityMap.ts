...
export const commoditySqlMap = {
  ...
  //添加一个新的商品资料信息
  saveCommodity:
    'insert into commodity(commodityId, categoryId, barcode, name, format,place, unitId, discountRate, costPrice, quantityUpperLimit, quantityLowerLimit, createDate, provideId, Status, remark) values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
  //获取指定页数的商品资料信息
  findCommodityByPage:
  ...
  /*getSelectCommodityListByPageAndCategoryId : 'SELECT c.commodityId, c.barcode, c.name, c.format,c.place,u.name as unitName, c.costPrice, c.discountRate, s.FactStoreNum, (c.quantityUpperLimit-s.FactStoreNum) as limitNum FROM commodity c, unit u, store s where c.commodityId =s.commodityId and c.Status != -2 and c.unitId = u.unitId and c.categoryId=?',*/
  ...
  //更新指定产品信息
  updateCommodityById:
    'update commodity set categoryId=?, barcode=?, name=?, format=?, unitId=?, discountRate=?, costPrice=?, quantityUpperLimit=?, quantityLowerLimit=?, provideId=?, Status=?, remark=? where commodityId=?',
  //删除指定id的商品信息
