...
export const $sql = {
  ...
  findBillCommodityCountByAdmin:
    'SELECT count(bi.infoId) AS count FROM bill_info bi, bill_list bl, commodity c WHERE bi.billId = bl.billId AND c.commodityId = bi.commodityId',
  //获取收银单中商品信息的总数（收银员）
  findBillCommodityCountByEmployee:
    'SELECT count(bi.infoId) AS count FROM bill_info bi, bill_list bl, commodity c WHERE bl.billUserId = ? AND bi.billId = bl.billId AND c.commodityId = bi.commodityId ',

  //获取收银单的总数(收银员)
  findBillCountByEmployee:
    'SELECT count(bl.billId) AS count FROM bill_list bl WHERE bl.billUserId = ?',
  //获取收银单的总数(管理员)
