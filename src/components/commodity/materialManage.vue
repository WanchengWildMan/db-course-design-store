        ...
        <el-row>
          <el-col :span="24" class="form-card">
            <div class="grid-content bg-purple">
              <material-find-form :propsModel="findInfo"></material-find-form>
            </div>
          </el-col>
          <el-col :span="24" class="form-card">
            <div class="grid-content bg-purple-light">
              <category-select labelText="选择商品类别" :selectModel="findInfo.categoryId" @on-result-change="onResultChange" ></category-select>
              <material-table
...
<script>
  ...
  export default {
    ...
    methods : {
      //下拉值改变时改变表格内容
      onResultChange(val) {
        this.findInfo.categoryId = val;
        this.findMaterialByPage();
      },
      findMaterialByPage () {
        this.$http.get('/api/commodity/findCommodityByPage', {
        	params : {
            findInfo : JSON.stringify(this.findInfo),
