<template>
  <el-table :data="tableData" border stripe fit style="width: 100%">
    ...
    <el-table-column prop="commodityId" label="商品编号" width="104">
    </el-table-column>
    <el-table-column prop="name" label="名称" width="168"> </el-table-column>
    <el-table-column prop="format" label="商品规格" width="100">
    </el-table-column>
    <el-table-column prop="retailPrice" label="零售价" width="80">
    </el-table-column>
    <el-table-column prop="factNum" label="库存数量" width="100">
    </el-table-column>
    <el-table-column label="库存金额" width="100" :formatter="totalPrice">
    </el-table-column>
    <el-table-column
      prop="Status"
      label="状态"
    ...
    <el-table-column label="操作" inline-template width="224">
      <div>
        <el-button type="info" size="small" @click="editMaterial(row)"
          >修改</el-button
        >
        <el-button
          size="small"
          @click="updateMaterialStatus(row.Status, row.commodityId)"
          >切换状态</el-button
        >
        <el-button
          type="danger"
          size="small"
          @click="deleteMaterial(row.name, row.commodityId)"
          >删除</el-button
        >
      </div>
    </el-table-column>
  ...
  methods: {
    ...
    editMaterial(row) {
      this.$router.push({
        ...
        params: { material: Object.assign({}, row) },
      });
    },
    //更新商品状态
    updateMaterialStatus(Status, materialId) {
      this.$http
        .put(
          "/api/commodity/updateCommodityStatusById",
          { Status: Status, commodityId: materialId },
          {}
        )
        .then((req) => {
          if (req.ok) {
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === "confirm") {
            ...
            self.$http
              .put(
                "/api/commodity/deleteCommodityById",
                { commodityId: materialId },
                {}
              )
              .then((req) => {
                if (req.ok) {
